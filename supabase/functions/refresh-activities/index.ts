...
function zonedDateTimeToUtcIso(dateStr: string, timeStr: string, timeZone: string): string {
  ...
  // Midnight can format as "24:00" with hour12:false in some environments -
  // normalize back to 0.
  const hour = get('hour') % 24;
  const asIfLocalMs = Date.UTC(get('year'), get('month') - 1, get('day'), hour, get('minute'));

  // guessUtcMs, reinterpreted in timeZone, tells us that zone's actual
  // offset from UTC at this moment (DST-aware) - adding it back to
  // guessUtcMs lands on the real UTC instant for the intended local time.
